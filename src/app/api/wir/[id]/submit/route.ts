...
import { submitWir, findWirProject } from "@/server/services/wir";
...
export const POST = api(
  async (req, meta, params) => {
    ...
    await submitWir(meta.ctx, projectId, params.id, parsed<{ comment?: string | null }>(req).comment ?? null);
    return ok({ ok: true });
  },
  { parse: actionSchema , permission: "wir:create"},
);
